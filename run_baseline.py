...
from vsign.modules.sync_batchnorm import convert_model
from vsign.engine.seq_scripts import seq_train, seq_eval, seq_feature_generation
...
class Processor():
    def __init__(self, arg):
        ...
        shutil.copy2('./src/vsign/modules/tconv.py', self.arg.work_dir)
        shutil.copy2('./src/vsign/modules/resnet.py', self.arg.work_dir)
    ...
    def init_fn(self, worker_id):
        # Calculate a unique seed for this worker based on the base random seed
        worker_seed = int(self.arg.random_seed) + worker_id

        # Set seeds for NumPy, Python's built-in random, and PyTorch (CPU and CUDA)
        # This ensures that random operations within this worker process are deterministic based on the worker_id and the base seed.
        np.random.seed(worker_seed)
        random.seed(worker_seed)
        torch.manual_seed(worker_seed)

        # If you use CUDA within the DataLoader workers themselves (less common for basic loading/augmentation but possible for some custom ops), uncomment the lines below:
    ...
    def build_dataloader(self, dataset, mode, train_flag):
        return torch.utils.data.DataLoader(
            ...
            pin_memory=True,
            worker_init_fn=self.init_fn # Ensures init_fn is called for each worker
        )
